...
console.log(`Maior índice registrado: ${maiorValor} (Região ${posicaoMaior.regiao + 1}, Dia ${posicaoMaior.dia + 1})`);
let contCritico = 0;
for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 5; j++) {
        if (matriz[i][j] > 300) {
...
console.log(`Região com menor média de poluição: Região ${regiaoMenorMedia + 1} (Média = ${menorMedia})`);